 ...
 *
 * Wrapped in PortalLayout so the bottom tab bar is always visible.
 * Vertical stacked prompt list (not horizontal scroll).
 * Safe-area aware for Dynamic Island / notch.
 */
...
export default function PortalAssistant() {
  ...
  return (
    <PortalLayout activeTab="assistant">
      {/* Full-height chat container — negative margin to reclaim PortalLayout padding */}
      <div
      ...
      >
        ...
        <div
          className="hidden md:flex flex-shrink-0 transition-all duration-200 overflow-hidden"
          style={{
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3">
            {messages.length === 0 ? (
              /* ── Empty state with VERTICAL stacked prompt list ── */
              <div className="flex flex-col items-center justify-center h-full gap-5 pb-4">
                ...
                {/* Vertical stacked prompt list */}
                <div className="w-full max-w-sm mx-auto flex flex-col gap-2 px-2">
                  {suggestedPrompts.map((prompt) => (
                    <button
                      key={prompt}
                      className="w-full text-left px-4 py-3 rounded-xl text-[13px] leading-snug transition-colors active:scale-[0.98]"
                      style={{
                    ...
                    >
                      <span className="flex items-center gap-2.5">
                        <Sparkles className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "#F5A623" }} />
                        {prompt}
                      </span>
                    </button>
                  ))}
